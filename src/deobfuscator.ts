...
class DecryptionFunctions {
	...
	static checkDecodeString(path: NodePath<t.FunctionDeclaration>, vmContext: vm.Context, funcObfStrings: string): string | undefined {
		const node = path.node;
		const funcExpr = node.body.body;
		...
		if (!t.isIdentifier(declarations[0].init?.callee, {name: funcObfStrings})) return;
	...
	static checkAdditionalDecodeString(
		path: NodePath<t.FunctionDeclaration>, vmContext: vm.Context,
		funcDecodeString?: string, funcFirstDecode?: string): [string, Binding] | undefined {
			if (!funcDecodeString && !funcFirstDecode) return;
			if (funcDecodeString && funcFirstDecode) return;
			...
			if (funcDecodeString && !t.isIdentifier(call.callee, {name: funcDecodeString})) return
			if (funcFirstDecode && !t.isIdentifier(call.callee, {name: funcFirstDecode})) return
			...
			if (!binding) {
				console.error("AdditionalDecodeString was found but with no references");
				path.stop();
				return;
			}

			path.remove();
			return [node.id.name, binding];
	}

	static checkShuffleObfuscatedStrings(path: NodePath<t.CallExpression>, vmContext: vm.Context, funcObfStrings: string): boolean | undefined {
		const node = path.node;

		if (node.arguments.length !== 2 || node.arguments.length !== 2) return;
...
class DecryptStrings {
	static decryptMapKeys(decryptIdentBinding: Binding, vmContext: vm.Context) {
		const references = decryptIdentBinding.referencePaths;
...
class MapReplacer {
	...
	public parseMap(path: NodePath<t.VariableDeclarator>): boolean | undefined {
		...
		node.init.properties = node.init.properties.filter((elemNode) => {
			...
			if (t.isFunctionExpression(elemNode.value)) {
				...
				this.decryptionMap[key] = elemNode.value.value as BinaryOperator;
			} else {
				console.error("Unknown value type occured in operations map!")
				return true;
			}
			return false;
		});
	...
	public replaceMap() {
		...
		for (const reference of references) {
			...
			// current indexing just a string
...
function deobfuscate(source: string) {
	...
	let funcDecodeString: string | undefined;
	let funcFirstDecode: [string, Binding] | undefined;
	...
	const parseDecryptFunctions = {
		FunctionDeclaration(path: NodePath<t.FunctionDeclaration>) {
			...
			if (!funcFirstDecode && funcDecodeString) {
				const result = DecryptionFunctions.checkAdditionalDecodeString(path, decryptCtx, funcDecodeString);
				if (result) {
					funcFirstDecode = result;
				}
			} else if (!funcSecondDecode && funcFirstDecode) {
				const result = DecryptionFunctions.checkAdditionalDecodeString(path, decryptCtx, undefined, funcFirstDecode[0]);
				if (result) {
					funcSecondDecode = result;
				}
			}
		},
		...
		CallExpression(path: NodePath<t.CallExpression>) {
			...
			if (DecryptionFunctions.checkShuffleObfuscatedStrings(path, decryptCtx, funcObfStrings)) {
				foundShiftFunc = true;
			}
		},
	};
	...
	DecryptStrings.decryptMapKeys(funcFirstDecode[1], decryptCtx);
	DecryptStrings.decryptMapKeys(funcSecondDecode[1], decryptCtx);
	...
	const processMap = {
		VariableDeclarator(path: NodePath<t.VariableDeclarator>) {
			...
			mapReplacer.simplifyCallExpr();

			mapReplacer.replaceMap();
